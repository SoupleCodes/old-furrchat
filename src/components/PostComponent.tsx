...
import { handleAttachments, getReplies, revisePost } from '../lib/RevisePost.ts';
...
//  _id,
  ...
  // Determine if the post has replies and extract reply text
  let shouldRender = getReplies(post) !== null;
  let postReply = getReplies(post)?.replyText || '';

  // Process and revise the post content
  let realPost = revisePost(post);
  let realUser = user;

  // Adjust user and post content if the user is "Discord"
  realUser = realUser === "Discord" ? post.split(':')[0] : realUser;
  realPost = realUser === "Discord" ? realPost.split(': ')[1] : realPost;

  // Fetch and set profile picture and avatar color
  let pfp = (fetchUserData(realUser, 'avatar'))?.toString() || '';
  let avatarColor = `#${(fetchUserData(realUser, 'avatar_color'))?.toString() || '000000'}`;

  // Use default profile picture if no avatar is available
  if (pfp === "") {
    let pfp_data: any = fetchUserData(realUser, 'pfp_data');
    pfp = pfp_data === -3 ? '/furrchat/assets/default_pfps/icon_guest-e8db7c16.svg' : defaultPFPS[34 - (pfp_data)];
  } else {
    pfp = `https://uploads.meower.org/icons/${pfp}`;
  }

  // Fetch profile picture for the reply if available
  let replyPFP = '';
  if (postReply) {
    let replyUser: any = postReply.split(' ')[0].replace("@", "");
    replyPFP = (fetchUserData(replyUser, 'avatar'))?.toString() || '';
    if (replyPFP === "") {
      let pfp_data: any = fetchUserData(replyUser, 'pfp_data');
      replyPFP = pfp_data === -3 ? '/furrchat/assets/default_pfps/icon_guest-e8db7c16.svg' : defaultPFPS[34 - (pfp_data)];
    } else {
  ...
  // Get the emoji prefix for the user if available
  let prefixEmoji = userEmojis[realUser] || "";
  let wholeReply = postReply?.substring(0, postReply?.lastIndexOf(" ")) || '';
      ...
      <div className="user">
        <span className="post-pfp-container">
          ...
          <img 
            ...
            width="48" 
            height="48" 
            style={{ padding: 5, boxShadow: `inset 0 0 3px ${avatarColor}`, border: `1.5px solid ${avatarColor}`, borderRadius: '10%' }} 
          />
        ...
        <p className="post-username-text"><strong>{prefixEmoji} {realUser}</strong></p>
      </div>
      <div className="post-content">
        {/* Display timestamp with edit status */}
        <div className="timestamp"><i>{realDate} {edited ? "(edited)" : ""}</i></div>
        <div className="postmessage">
          {shouldRender && (
            <div className="reply"><i>
              <img src={replyPFP} alt="reply pfp" width="16" height="16" style={{ paddingRight: 5 }} />
              {wholeReply}
            </i></div>
          )}
          {/* Render the post content with syntax highlighting for code blocks */}
          <ReactMarkdown 
